function [EEG,record] = edf2eeglab(fname, varargin)
...
EEG.xmax=hdr.records-1/EEG.srate;
EEG.xmin=0;
EEG.nbchan=hdr.ns;
EEG.pnts=size(record,2);


for ii=1:hdr.ns
    EEG.chanlocs(ii).labels=char(hdr.label(ii));
    EEG.chanlocs(ii).ref=char([]);
    EEG.chanlocs(ii).theta=[];EEG.chanlocs(ii).radius=[];
    EEG.chanlocs(ii).X=[];EEG.chanlocs(ii).Y=[];EEG.chanlocs(ii).Z=[];
    EEG.chanlocs(ii).sph_theta=[];EEG.chanlocs(ii).sph_phi=[];EEG.chanlocs(ii).sph_radius=[];
    EEG.chanlocs(ii).type=char([]);
    EEG.chanlocs(ii).urchan=[];
end
EEG.ref=('Ref');

 EEG.filepath='';
 EEG.comments=strcat('Original file: ', EEG.filepath);
  EEG.history=["EEG.etc.eeglabvers = '14.1.2'; % this tracks which version of EEGLAB is being used, you may ignore it"];
  EEG.subject=hdr.patientID;
  EEG.group=[''];
  EEG.saved='no';  EEG.datfile='';
  EEG.trials=1;
%   
% 
  EEG.times=[0:1/EEG.srate:EEG.pnts./EEG.srate-1/EEG.srate];
EEG.etc.T0=strcat(hdr.recordID," ",hdr.starttime);
EEG.icawinv =[];EEG.icaweights =[];EEG.icaact = [];EEG.icawinv = [];EEG.icasphere = [];EEG.icaweights = [];EEG.icachansind = [];EEG.filename = fname;
EEG.data=record;
